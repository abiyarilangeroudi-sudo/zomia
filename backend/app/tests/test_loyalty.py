...
def test_owner_cannot_manage_another_owner_mission(client: TestClient) -> None:
    owner_token, _ = register_owner(client, "owner1@example.com", "Owner One Cafe")
    other_owner_token, other_business_id = register_owner(
        client, "owner2@example.com", "Owner Two Cafe"
    )
    mission = create_mission(
        client,
        other_owner_token,
        other_business_id,
        name="Other Owner Mission",
    )

    list_response = client.get(
        f"/api/v1/owner/missions?business_id={other_business_id}",
        headers=auth(owner_token),
    )
    update_response = client.patch(
        f"/api/v1/owner/missions/{mission['id']}?business_id={other_business_id}",
        json={"name": "Forbidden Update", "point_value": 2},
        headers=auth(owner_token),
    )
    archive_response = client.patch(
        f"/api/v1/owner/missions/{mission['id']}/active?business_id={other_business_id}",
...
def test_owner_cannot_manage_another_owner_reward_template(client: TestClient) -> None:
    owner_token, _ = register_owner(client, "owner1@example.com", "Owner One Cafe")
    other_owner_token, other_business_id = register_owner(
        client, "owner2@example.com", "Owner Two Cafe"
    )
    template = create_reward_template(
        client,
        other_owner_token,
        other_business_id,
        name="Other Owner Reward",
    )

    list_response = client.get(
        f"/api/v1/owner/reward-templates?business_id={other_business_id}",
        headers=auth(owner_token),
    )
    update_response = client.patch(
        f"/api/v1/owner/reward-templates/{template['id']}?business_id={other_business_id}",
        json={"name": "Forbidden Update", "gift_name": "Forbidden", "valid_days": 45},
        headers=auth(owner_token),
    )
    archive_response = client.patch(
        f"/api/v1/owner/reward-templates/{template['id']}/active?business_id={other_business_id}",
        json={"is_active": False},
        headers=auth(owner_token),
    )
    delete_response = client.delete(
...
def test_owner_can_archive_used_reward_template_after_campaign_expires(
    client: TestClient,
) -> None:
    owner_token, business_id = register_owner(client, "owner@example.com")
    mission = create_mission(client, owner_token, business_id, name="Buy Coffee")
    ...
    assert list_response.status_code == 200
    listed_template = next(item for item in list_response.json() if item["id"] == template["id"])
    assert listed_template["can_archive"] is True
